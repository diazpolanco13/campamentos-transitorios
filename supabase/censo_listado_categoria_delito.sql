-- Listado red: expone categorías de delito + filtro p_categoria_delito.
...
drop function if exists public.censo_listado_red_conteo(boolean, text, text, text, boolean, boolean, boolean);
...
create function public.censo_listado_red_conteo(
  ...
  p_categoria_delito text default null
)
returns bigint
language plpgsql
stable
...
declare
  ...
  v_categoria text := nullif(trim(coalesce(p_categoria_delito, '')), '');
  ...
  return (
    ...
    where
      ...
      and (
        v_categoria is null
        or (
          p_solicitado is true
          and r.categoria_delito_solicitado = v_categoria
        )
        or (
          p_registro_policial is true
          and r.categoria_delito_registro = v_categoria
        )
        or (
          p_solicitado is distinct from true
          and p_registro_policial is distinct from true
          and (
            r.categoria_delito_solicitado = v_categoria
            or r.categoria_delito_registro = v_categoria
          )
        )
      )
      and (
...
returns table (
  ...
  categoria_delito_solicitado text,
  categoria_delito_registro text
)
language plpgsql
stable
security definer
...
begin
  ...
  select
    ...
    r.categoria_delito_solicitado,
    r.categoria_delito_registro
  from public.censo_registros r
  inner join public.centros c on c.id = r.centro_id and not c.deleted
  where
    (p_centro_id is null or p_centro_id = '' or p_centro_id = 'todos' or r.centro_id = p_centro_id)
...
revoke all on function public.censo_listado_red_paginado(boolean, integer, integer, text, text, text, text, boolean, boolean, boolean, text) from public, anon;
grant execute on function public.censo_listado_red_paginado(boolean, integer, integer, text, text, text, text, boolean, boolean, boolean, text) to authenticated;

revoke all on function public.censo_listado_red_conteo(boolean, text, text, text, boolean, boolean, boolean, text) from public, anon;
grant execute on function public.censo_listado_red_conteo(boolean, text, text, text, boolean, boolean, boolean, text) to authenticated;
