...
-- censo_importar_lote: jefe menores + flags seguridad monotónicos (no wipe en update)
-- Extraído del patrón de censo_rapido; grants iguales al final.
      ...
      if v_hay_existente then
        ...
        if v_existente.procesado then
          update public.censo_registros r set
            ...
            -- Flags seguridad monotónicos: import sin columnas NO apaga alertas previas
            ...
            tipo_registro_policial = case
              when coalesce(v_tipo_registro_policial, '') <> '' then v_tipo_registro_policial
              else r.tipo_registro_policial
            end,
            observaciones_seguridad = case
              when coalesce(v_observaciones_seguridad, '') <> '' then v_observaciones_seguridad
            ...
            jefe_documento = coalesce(v_jefe_documento, r.jefe_documento),
            parentesco_jefe = case
              when coalesce(v_parentesco_jefe, '') <> '' then v_parentesco_jefe
              else r.parentesco_jefe
            end
          ...
          update public.censo_registros r set
            ...
            parentesco_jefe = v_parentesco_jefe
          where r.id = v_existente.id;
        end if;
